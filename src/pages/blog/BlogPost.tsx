...
export default function BlogPost() {
  ...
  const handleShare = (platform: "twitter" | "linkedin" | "facebook" | "whatsapp") => {
    share({ platform, url: window.location.href, text: "Check out this blog post on Golvia \n" });
  };

  return (
    <AccessLayout path={PATHS.BLOG}>
      {blog?.data && (
        <Box maxWidth={{ xs: "100%", md: "80%" }} justifySelf="center" mt={6} mb={8}>
          ...
          <Typography variant="h$32" fontSize="40px" fontWeight={600} lineHeight="48px" mb={5}>
            {blog.data?.title}
          </Typography>
          <Box borderRadius="6px" overflow="hidden">
            <img src={blog.data.images?.[blog?.data?.images?.length - 1]?.imageUrl} height={400} />
            <Typography variant="p$14" fontWeight={500} mt={1.5}>
              image via: {blog.data.images?.[blog?.data?.images?.length - 1]?.mediaCredit}
            </Typography>
          </Box>
          <Stack direction={{xs: "column-reverse", sm: "row" }}spacing={8} mt={5}>
            <Stack>
            ...
            <Typography variant="p$18" fontSize="20px" flex={1} sx={{fontSize:{md:'20px', xs:'16px'}}}>
      ...
      {!blog?.data && (
        <Stack mt={4}>
          <Stack spacing={2}>
            ...
            <Skeleton variant="rectangular" sx={{ fontSize: "1rem" }} width="100%" height={350} />
