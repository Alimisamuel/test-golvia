import { Box, Button, Typography, Avatar, InputLabel, Divider, Skeleton } from "@mui/material";
import BackdropLoader from "components/loaders/Backdrop";

import { selectAuth, updateAsync } from "pages/auth/slice";
...
const ProfileCompleted = () => {
  ...
  const { firstName, lastName, profileDetails, sportType, asset } = useAuthDetails();
  return (
    <>
      ...
      <Box
        sx={{
          ...
          flexDirection: { lg: "row", md: "row", sm: "column-reverse", xs: "column" },
          rowGap: 2,
        }}
      >
        <Box>
        ...
        <Box>
          ...
          >
            <Box sx={{ display: "flex", alignItems: "center", columnGap: 3, p: 3 }}>
              <div className="relative">
                <Avatar src={asset?.profilePictureUrl} sx={{ width: 60, height: 60 }} />
              ...
              <Box>
                {authState?.loading ? (
                  ...
                  <InputLabel sx={{ fontWeight: 500, fontSize: "18px", color: "#000" }}>
                    {`${firstName} ${lastName}`}
                  </InputLabel>
                )}
            ...
            <Box sx={{ p: 3 }}>
              <Typography sx={{ color: "text.disabled" }}>
                Location: <span className="text-black">{profileDetails?.address || "--"}</span>
              </Typography>
