          ...
          <div className=" w-1/4 h-full">
            <div className="bg-white rounded-2xl" style={{ border: "0.5px solid #dfdeda " }}>
              <div className="px-8 pt-8">
                ...
                <div className="space-y-1 my-4">
                  ...
                  <h4 className="text-sm">
                    {profileType === "ATHLETES" ? "Athlete" : convertToTitleCase(profileType) || ""}
                  </h4>
                </div>
              </div>
              <hr />
              <div className="px-8 pb-8 mt-4 flex items-start justify-between w-full">
                ...
                <CircularLoader value={isCompleted || isProfileEdit ? 100 : 40} />
              </div>
            </div>
    <ActivityStat/>
          </div>
        ...
        >
          ...
          {isCompleted ? (
            ...
            <>
              {!isMobile && (
                <>
                  <Stack direction="row" spacing={{ xs: 4, lg: 8 }} paddingX={4} paddingY={3}>
                    {Object.entries(sections).map(([key, { label, completed }], index) => (
              ...
              {pType.SCOUT === profileType && <ScoutComponent isProfileEdit={isProfileEdit} />}
              {pType.CLUB === profileType && <TeamComponent isProfileEdit={isProfileEdit} />}
              {pType.FANBASE === profileType && <FanBaseComponent isProfileEdit={isProfileEdit} />}
            </>
          )}
        </Stack>
      </Stack>
...
const AtheleteComponent = (props: editPayload) => {
  const { currentSectionKey, onNext, onPrevious, onComplete } = useSectionHook("ATHLETES");
  return (
    <Athletes
...
const ScoutComponent = (props: editPayload) => {
  const { currentSectionKey, onNext, onPrevious, onComplete } = useSectionHook("SCOUT");
  return (
    <Scout
      currentSectionKey={currentSectionKey}
      onNext={onNext}
...
const TeamComponent = (props: editPayload) => {
  const { currentSectionKey, onNext, onPrevious, onComplete } = useSectionHook("TEAM");
  return (
    <Club
      currentSectionKey={currentSectionKey}
      onNext={onNext}
...
const FanBaseComponent = (props: editPayload) => {
  const { currentSectionKey, onNext, onPrevious, onComplete } = useSectionHook("FANBASE");
  return (
    <Fanbase
      currentSectionKey={currentSectionKey}
      onNext={onNext}
